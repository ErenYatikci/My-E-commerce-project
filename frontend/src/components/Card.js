            ...
            <>
                <article class="fl w-100 w-50-m  w-25-ns pa2-ns">
                    ...
                    <div class="db bc-center w-100 h-200 br2 br--top ">
                        <img src={`http://${deal.image}`} height="350" width="400" alt={deal.name}
                            class="" />
                        {/* <h1 class="f2 pa2 f2-ns mb0 black-90 tl">From: {deal.source}</h1>
                            <h3 class="f2 pa2 f2-ns mb0 black-90 tl">To: {deal.destination}</h3> */}
                    </div>
                    <h2 className="f3 tl">Original Price: <span style={{ color: "#820001", fontWeight: "bolder" }}>{deal.price.toFixed(2)} TL</span></h2>

                    {/* <h1>Hurry up!!</h1>
                        <div className="f3">Available for: {date.minutes} minutes, {date.seconds} seconds</div> */}
                    <div class="ph2 ph0-ns pb3 pa1   db">
                        <p className="btn btn-danger tc white b pa-1 f4" style={{ borderRadius: 40 }}>Discounted Price: {Math.abs(deal.discount).toFixed(2)} TL</p>
                        <br></br>
                        <button className="ant-btn ant-btn-primary" style={{ fontFamily: "'Open Sans Condensed', sans-serif",background: "orange",borderColor: "white", fontSize: 20, height: 50 }} onClick={updateCart}>Add to Cart</button>
                    </div>
                </article>
            </>
        </section>
